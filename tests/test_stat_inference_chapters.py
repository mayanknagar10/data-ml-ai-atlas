...
class StatisticalInferenceChapterTests(unittest.TestCase):
    def test_every_statistical_inference_lesson_is_a_verified_full_chapter(self):
        data = json.loads((ROOT / "source" / "atlas.json").read_text(encoding="utf-8"))
        lessons = [lesson for lesson in data["lessons"] if lesson["module"] == "stat-inference"]
        ...
        for lesson in lessons:
            with self.subTest(slug=lesson["slug"]):
                ...
                for visual in lesson["visuals"]:
                    ...
                    if visual.get("type") == "matrix":
                        self.assertTrue(visual.get("rows"))
                        self.assertTrue(visual.get("cols"))
                        self.assertTrue(visual.get("cells"))
                self.assertIn("lab", lesson)
                self.assertEqual(lesson["lab"].get("language"), "python")
