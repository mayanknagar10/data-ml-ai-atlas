...
class ProjectCaseStudyTests(unittest.TestCase):
    def test_every_project_case_study_is_a_verified_full_chapter(self):
        data = json.loads((ROOT / "source" / "atlas.json").read_text(encoding="utf-8"))
        lessons = [lesson for lesson in data["lessons"] if lesson["module"] == "project-defense"]
