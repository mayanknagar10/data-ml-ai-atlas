...
# ---- Verify it ----
assert tsne.shape==(len(X),2) and pca.shape==(len(X),2)
assert np.isfinite(tsne).all() and np.isfinite(pca).all()
assert np.allclose(tsne,tsne_again)
assert .5<=tw_tsne<=1 and .5<=tw_pca<=1
assert tw_tsne>0.9
assert not np.allclose(tsne,pca)
