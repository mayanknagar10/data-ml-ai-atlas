...
# ---- Verify it ----
assert len(stumps)==30
assert all(b<=a+1e-12 for a,b in zip(losses,losses[1:]))
reconstructed=np.full(len(y),base)
for stump in stumps: reconstructed+=rate*stump_predict(X,stump)
assert np.allclose(reconstructed,scratch_pred)
assert losses[-1]<0.2*losses[0]
assert np.mean((y-sk_pred)**2)<0.2*np.var(y)
assert np.isfinite(sk_pred).all()
