...
# ---- Verify it ----
assert topo(graph)==['stg_items','stg_orders','int_lines','fct_lines']
assert all(con.execute('SELECT built FROM '+relation('pr42',node)).fetchone()==(1,) for node in graph)
assert relation('prod','fct_lines')!='pr42__fct_lines'
try: topo({'a':['b'],'b':['a']})
