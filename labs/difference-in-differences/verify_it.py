...
# ---- Verify it ----
assert abs(did-effect)<.04
assert abs(interaction-did)<1e-12 and abs(direct-did)<1e-12
assert abs((post_gap-placebo_pre_gap)-did)<1e-12
assert abs(placebo_pre_gap-2)<.05
assert len(changes)==units
